...
3. **Run intent classifier** (gap-12 §6) — heuristic-first; LLM-residual within the 200 ms budget.
   ...
   - **Skip this step entirely if `force_skip_classifier=true`** — only callable by principals holding the `tenant_admin` capability; refuse with `403 forbidden` otherwise. The bypass is the escalate-review approval path (deployment §6.3): a payload that previously escalated does not re-escalate on re-submission. Two distinct audit rows live around this verb, separated by phase:
     - **`force_skip_classifier_invoked`** — the bypass-invocation row, written at P2 by `remember` itself whenever the parameter is `true`. Shape: `audit_log` columns `(tenant_id, verb='remember', principal, action='force_skip_classifier_invoked', payload_json={caller_tag, request_idempotency_key})`. This is the only `audit_log` row `remember` writes at P2.
     - **`review_approved{staged_id, reviewer_principal, reviewed_at, reason}`** — the upstream approval row that *justifies* a later `force_skip_classifier=true` re-submission. Owned by deployment §6.3 (and indexed in §5 row `remember (escalate)`); it has its own column shape and lands at P7 alongside the review workflow. api §3.1 does not redefine it.
4. **Branch on class**:
